...
void runBenchmark(std::size_t num_events){
    ...
    //pre-resolve symbol to SymbolId: no string ops in the hot loop
    SymbolId foo = engine.resolveSymbol("FOO");
    ...
    for(std::size_t i = 0; i < num_events; ++i){
        if(!live_orders.empty() && cancel_dist(rng) == 0){
            ...
            engine.cancel(foo, id);

            live_orders[idx] = live_orders.back();
            live_orders.pop_back();
        }
        else{
            //new limit (SymbolId-based: zero string allocation)
            Side side = (side_dist(rng) == 0 ? Side::Buy : Side::Sell);
            Price price = price_dist(rng);
            Qty qty = qty_dist(rng);

            OrderId id = engine.newLimit(foo, UserId{1}, side, price, qty);
...
void runAsyncBenchmark(std::size_t num_events){
    ...
    //pre-resolve symbol
    SymbolId foo = async_eng.engine().resolveSymbol("FOO");
    ...
    for(std::size_t i = 0; i < num_events; ++i){
        ...
        if(!live_orders.empty() && cancel_dist(rng) == 0){
            ...
            //new limit (InternalEvent: zero allocation, value-based queue push)
            ie.type = EventType::NewLimit;
            ie.symbol = foo;
            ie.side = (side_dist(rng) == 0 ? Side::Buy: Side::Sell);
            ie.price = price_dist(rng);
            ie.qty = qty_dist(rng);
            ie.tif = TimeInForce::GFD;
            async_eng.submit(ie);
        }
    }
    ...
    if(auto stats = async_eng.engine().bookStats("FOO")){
...
void runInteractiveSync(){
    ...
    while(std::getline(std::cin , line)){
        ...
        if(!trimmed.empty() && trimmed[0] == 'D'){
            ...
            const auto* book = engine.findBook(symbol);
...
int main(int argc, char** argv){
    ...
    MatchingEngine engine([](const Trade& t){
        std::cout << "TRADE symbol=" << t.symbol_name
                  << " px="  << t.price
                  << " qty=" << t.qty
                  << " buy=" << t.buy_id
                  << " sell="<< t.sell_id
    ...
    {
        std::cout << "\n--- IOC test (BAR) ---\n";
        engine.newLimit("BAR", Side::Sell, 100, 50);
    ...
    {
        std::cout << "\n--- FOK test (BAZ) ---\n";
        engine.newLimit("BAZ", Side::Sell, 100, 50);
        ...
        //async engine demo
    {
        ...
        AsyncMatchingEngine async_eng([](const Trade& t) {
            std::cout << "ASY TRADE symbol=" << t.symbol_name
                      << " px="  << t.price
                      << " qty=" << t.qty
                      << " buy=" << t.buy_id
                      << " sell="<< t.sell_id
    ...
    std::cout << "\n--- Running benchmark ---\n";
    runBenchmark(2'000'000);

    std::cout << "\n--- Running async benchmark ---\n";
    runAsyncBenchmark(2'000'000);
